...
import { pageOrder, findItem } from '@/lib/nav';
...
export function Pager({ currentRoute }: Props) {
  ...
  const prev = idx > 0 ? findItem(pageOrder[idx - 1]) : null;
  const next = idx >= 0 && idx < pageOrder.length - 1 ? findItem(pageOrder[idx + 1]) : null;
